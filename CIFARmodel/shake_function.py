...
shake = ShakeFunction.apply


def get_alpha_beta(batch_size, shake_config, device, lmd = 0.):
    forward_shake, backward_shake, shake_image = shake_config

    if forward_shake and not shake_image:
        alpha = torch.rand(1) * (1 - lmd) + lmd / 2.
    elif forward_shake and shake_image:
        alpha = torch.rand(batch_size).view(batch_size, 1, 1,
                                            1) * (1. - lmd) + lmd / 2.
    else:
        alpha = torch.FloatTensor([0.5])

    if backward_shake and not shake_image:
        beta = torch.rand(1) * (1 - lmd) + lmd / 2.
    elif backward_shake and shake_image:
        beta = torch.rand(batch_size).view(batch_size, 1, 1,
                                           1) * (1. - lmd) + lmd / 2.
    else:
