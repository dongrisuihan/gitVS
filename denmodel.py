...
from CIFARmodel_lwj.densenet import densenet
...
parser.add_argument('--savefile',
                    type=str,
                    default='./savefile/cifar100/densenet/densenet40_multigpu')
...
class Cifar_VGG:
    def __init__(self):
        ...
        self.net = densenet(num_init_features=nif, arch_set=arch, num_classes=self.num_classes, small_inputs=True)
        ...
        self.prenet = {
            'net': densenet(num_init_features=nif, arch_set=arch, num_classes=self.num_classes, small_inputs=True),
            'acc': 0,
            'cost': 0,
    ...
    def run(self):
        ...
        def resume():
            ...
            self.prune_idx = checkpoint['prune_idx'] if 'prune_idx' in checkpoint.keys() else 0
        ...
        def stream(epoch):
            ...
            def calstream(current, prenet):
                ...
                self.base_prune = np.min(st[st > 0]) * 0.05 + 1
                st[:] = 0
                s1, s2 = st.shape
        ...
        def savemodel(epoch, name='final', pi=0):
            logger.info('Saving...')
            state = {
            ...
            if epoch == self.stream_epoch-1:
                torch.save(state,
                       self.savefile_checkpoint + '/ckpt_stream_epoch' + name + '.pth')
            else:
                torch.save(state,
