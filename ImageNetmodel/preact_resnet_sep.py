...
class PreActBlock(nn.Module):
    ...
    def __init__(self, in_planes, arch, stride=1, gate=[]):
...
class PreActBottleneck(nn.Module):
    ...
    def __init__(self, in_planes, arch, stride=1, gate=[]):
        ...
        self.bn2 = nn.BatchNorm2d(arch[0])
        self.conv2 = nn.Conv2d(arch[0], arch[0], kernel_size=3, stride=stride, padding=1, bias=False)
...
class PreActResNet(nn.Module):
    def __init__(self, block, archs, num_classes=10):
        ...
        self.layer4 = self._make_layer(block, self.arch_set[3], stride=2, gate=self.gate[3])

        self.bn = nn.BatchNorm2d(self.arch_set[3, 1])
        self.linear = nn.Linear(self.arch_set[3, 1], num_classes)
    ...
    def _make_gate(self, device='cuda'):
        ...
        for gs in self.arch_set:
            ...
            for g in gs:
                ...
                gate.append(Variable(torch.ones(1, g, 1, 1).cuda()))
    ...
    def cost(self):
        if self.block == PreActBottleneck:
            ...
            gate = self.gate_set
            for g in gate:
                p = g[1] * g[0] * 2 + g[0] * g[0] * 3 * 3
                p = p * g[2] - g[1] * g[0]
                f = p * a
                para += p
                flop += f
                a /= 4
            p00 = gate[0][1] * gate[0][1] + gate[0][1] * 3 * 7 * 7
            f00 = gate[0][1] * gate[0][1] * 3 * 3 + 112 * 112 * gate[0][1] * 3 * 7 * 7
            p01 = gate[0][1] * gate[1][1] * 2
            f01 = (56 * 56 + 28 * 28) * gate[0][1] * gate[1][1]
            p12 = gate[1][1] * gate[2][1] * 2
            f12 = (28 * 28 + 14 * 14) * gate[1][1] * gate[2][1]
            p23 = gate[2][1] * gate[3][1] * 2
            f23 = (14 * 14 + 7 * 7) * gate[2][1] * gate[3][1]
            p33 = gate[3][1] * self.num_classes
            f33 = p33
            para += p00 + p01 + p12 + p23 + p33
            flop += f00 + f01 + f12 + f23 + f33
            return para, flop
